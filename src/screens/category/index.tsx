...
type CategoryProps = {
  navigation: RootDrawerNavigationProp<'Category'>; // Specify the type for the navigation prop
};
...
const Category: React.FC<CategoryProps> = ({ navigation }) => {
  ...
  const Fields = (
    ...
    if (item.field === 'TextField') {
      const stringValue = item.value !== undefined ? String(item.value) : undefined;
      return (
        <TextField
          key={itemIndex}
          value={stringValue}
    ...
    if (item.field === 'NumberField') {
      const numberValue = typeof item.value === 'number' ? item.value : undefined;
      return (
        <NumberField
          key={itemIndex}
          value={numberValue}
